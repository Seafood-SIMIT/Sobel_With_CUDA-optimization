...
**/
//Sobel算子边缘检测核函数
__global__ void sobelInCuda(unsigned char *dataIn, unsigned char *dataOut, int imgHeight, int imgWidth, int *dev_sobel_x, int *dev_sobel_y)
{
    //用单thread操作
    ...
    while(offset < (imgHeight - 2) * (imgWidth - 2))
    {
        ...
        Gx = dev_sobel_x[0] * dataIn[(yIndex) * imgWidth + xIndex] + dev_sobel_x[1] * dataIn[(yIndex +1 ) * imgWidth + xIndex] + dev_sobel_x[2] * dataIn[(yIndex + 2) * imgWidth + xIndex]
            + dev_sobel_x[3] * dataIn[(yIndex) * imgWidth + xIndex + 1] + dev_sobel_x[4] * dataIn[(yIndex+1) * imgWidth + xIndex + 1] + dev_sobel_x[5] * dataIn[(yIndex + 2) * imgWidth + xIndex + 1]
            + dev_sobel_x[6] * dataIn[(yIndex) * imgWidth + xIndex + 2] + dev_sobel_x[7] * dataIn[(yIndex+1) * imgWidth + xIndex + 2] + dev_sobel_x[8] * dataIn[(yIndex + 2) * imgWidth + xIndex + 2];
        Gy = dev_sobel_y[0] * dataIn[(yIndex) * imgWidth + xIndex] + dev_sobel_y[1] * dataIn[(yIndex +1 ) * imgWidth + xIndex] + dev_sobel_y[2] * dataIn[(yIndex + 2) * imgWidth + xIndex]
            + dev_sobel_y[3] * dataIn[(yIndex) * imgWidth + xIndex + 1] + dev_sobel_y[4] * dataIn[(yIndex+1) * imgWidth + xIndex + 1] + dev_sobel_y[5] * dataIn[(yIndex + 2) * imgWidth + xIndex + 1]
            + dev_sobel_y[6] * dataIn[(yIndex) * imgWidth + xIndex + 2] + dev_sobel_y[7] * dataIn[(yIndex+1) * imgWidth + xIndex + 2] + dev_sobel_y[8] * dataIn[(yIndex + 2) * imgWidth + xIndex + 2];
...
*@return:void
*@function：图像读取、高斯滤波sobel_y[8]和初始化
*@para：None
*其他要注意的地方
**/
...
void sobelGPUPixel()
{
    ...
    cudaMalloc((void**)&dev_sobel_x, 9 * sizeof(int));
    cudaMalloc((void**)&dev_sobel_y, 9 * sizeof(int));
    //导入内存
    cudaMemcpy(dev_in, g_gaussImage.data, g_imgHeight * g_imgWidth * sizeof(unsigned char), cudaMemcpyHostToDevice);
    cudaMemcpy((void*)dev_sobel_x, (void*)sobel_x, 3 *3* sizeof(int), cudaMemcpyHostToDevice);
    cudaMemcpy((void*)dev_sobel_y, (void*)sobel_y, 3 *3* sizeof(int), cudaMemcpyHostToDevice);
    //调用核函数
    
    //定义block thread范围
    dim3 blocks((int)((g_imgWidth+31)/32), (int)(g_imgHeight+31)/32);
    dim3 threads(16, 16);
    sobelInCuda<< <blocks,threads>> >(dev_in, dev_out, g_imgHeight, g_imgWidth, dev_sobel_x, dev_sobel_y);
